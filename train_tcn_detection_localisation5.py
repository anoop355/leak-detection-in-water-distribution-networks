...
for ep in range(EPOCHS):
    ...
    for X, leak_count, pipe_t, pos_t, size_t, slot_mask in train_loader:
        ...
        # 1) leak count loss (unmasked — operates at scenario level)
        ...
        # 3) size loss — masked to real leak slots only
...
print("[OK] Saved multileak_tcn_bundle.pt (local runtime)")

torch.save(save_bundle, DRIVE_SAVE_PATH)
