...
def is_no_leak(labels: dict):
    return int(labels.get("leak_present", 0)) == 0

def encode_labels_from_json(labels: dict):
    """
    Converts a labels.json (supporting 0–3 simultaneous leaks) into targets.

    Returns:
      detect:  int   0=no-leak, 1=leak
      leaks:   list of MAX_LEAKS tuples (pipe_t, pos_t, size_t)
               padded with (PIPE_NONE_IDX, 0.0, SIZE_NONE_IDX) for unused slots
    """
    detect = int(labels.get("leak_present", 0))

    slots = []
    if detect == 1:
        raw_leaks = labels.get("leaks", [])
        # Sort by pipe_id for consistent slot ordering
        raw_leaks = sorted(raw_leaks, key=lambda l: int(l["pipe_id"]))
        for lk in raw_leaks[:MAX_LEAKS]:
            pipe_t = int(lk["pipe_id"]) - 1           # 1..5 -> 0..4
            pos_t  = float(lk["position"])
            sl     = str(lk.get("size_level", "S")).upper()
            size_t = SIZE_TO_IDX.get(sl, 0)
            slots.append((pipe_t, pos_t, size_t))

    # Pad remaining slots with NONE
    while len(slots) < MAX_LEAKS:
        slots.append((PIPE_NONE_IDX, 0.0, SIZE_NONE_IDX))
...
# ============================================================
def get_all_folders(dataset_root: str):
    """Return all valid scenario folders (have signals.csv + labels.json)."""
    folders = []
    for d in sorted(os.listdir(dataset_root)):
        path = os.path.join(dataset_root, d)
        sig  = os.path.join(path, "signals.csv")
        lab  = os.path.join(path, "labels.json")
        if os.path.isdir(path) and os.path.isfile(sig) and os.path.isfile(lab):
            folders.append(path)
    return folders

def split_folders(folders, train_frac=TRAIN_FRAC, val_frac=VAL_FRAC, seed=SEED):
    """Stratified random split by leak_present."""
    rng = random.Random(seed)

    leak_folders    = []
    no_leak_folders = []
    for f in folders:
        labels = load_labels(os.path.join(f, "labels.json"))
        (no_leak_folders if is_no_leak(labels) else leak_folders).append(f)
    ...
    l_tr, l_va, l_te     = _split(leak_folders)
    n_tr, n_va, n_te     = _split(no_leak_folders)
    return l_tr + n_tr, l_va + n_va, l_te + n_te
...
class ScenarioWindowDataset(Dataset):
    def __init__(self, folders, baseline_template, mu, sigma, window=12, stride=1):
        ...
        self.targets       = []     # each entry: (detect, [(pipe_t, pos_t, size_t) * MAX_LEAKS])
        self.names         = []
        self.window_counts = []
        self.cum_counts    = [0]
    ...
    def __getitem__(self, idx):
        ...
        detect, leaks = self.targets[scenario_idx]
        # leaks: list of MAX_LEAKS tuples (pipe_t, pos_t, size_t)
        pipe_ts = torch.tensor([lk[0] for lk in leaks], dtype=torch.long)     # (MAX_LEAKS,)
        pos_ts  = torch.tensor([lk[1] for lk in leaks], dtype=torch.float32)  # (MAX_LEAKS,)
        size_ts = torch.tensor([lk[2] for lk in leaks], dtype=torch.long)     # (MAX_LEAKS,)

        return (
            torch.tensor(x, dtype=torch.float32),           # (W, N, 2)
            torch.tensor(detect, dtype=torch.long),         # scalar
            pipe_ts,                                        # (MAX_LEAKS,)
            pos_ts,                                         # (MAX_LEAKS,)
            size_ts,                                        # (MAX_LEAKS,)
        )
...
class MultiLeakSTGCN(nn.Module):
    """
    Shared ST-GCN backbone + multi-leak output heads (up to MAX_LEAKS simultaneous leaks):
      - detect:       2 classes (0=no-leak, 1=leak)
      - pipe[k]:      6 classes (5 pipes + NONE)  for each leak slot k
      - size[k]:      4 classes (S/M/L + NONE)    for each leak slot k
      - pos[k]:       scalar regression            for each leak slot k

    Backbone architecture (STBlocks + TemporalAttentionPool) is unchanged from v5.
    """
    def __init__(self, adj_matrix):
        ...
        # One set of heads per leak slot
        self.pipe_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(head_in, head_hidden),
                nn.ReLU(),
                nn.Dropout(DROPOUT),
    ...
    def forward(self, x):
        ...
        pipe_logits_all = [h(z) for h in self.pipe_heads]              # MAX_LEAKS × (B, PIPE_CLASSES)
        size_logits_all = [h(z) for h in self.size_heads]              # MAX_LEAKS × (B, SIZE_CLASSES)
        pos_pred_all    = [h(z).squeeze(1) for h in self.pos_heads]    # MAX_LEAKS × (B,)

        return detect_logits, pipe_logits_all, size_logits_all, pos_pred_all
...
def scenario_level_detection_accuracy(model, dataset, batch_size=256):
    ...
    for i in range(len(dataset.features)):
        ...
        detect = dataset.targets[i][0]  # detect is targets[i][0]
...
def main():
    ...
    all_folders = get_all_folders(DATASET_ROOT)
    if len(all_folders) == 0:
        raise RuntimeError(f"No valid scenario folders found in {DATASET_ROOT}")

    train_folders, val_folders, test_folders = split_folders(all_folders)
    ...
    model = MultiLeakSTGCN(adj).to(DEVICE)
    ...
    # Loss functions.
    # Pipe and size use reduction="none" so the leak-slot mask can be applied
    # before averaging, excluding absent-leak slots from localisation gradients.
    ...
    for ep in range(EPOCHS):
        ...
        for x, detect, pipe_ts, pos_ts, size_ts in train_loader:
            # pipe_ts / pos_ts / size_ts: (B, MAX_LEAKS)
            x       = x.to(DEVICE, non_blocking=True)
            detect  = detect.to(DEVICE, non_blocking=True)
            pipe_ts = pipe_ts.to(DEVICE, non_blocking=True)
            pos_ts  = pos_ts.to(DEVICE,  non_blocking=True)
            size_ts = size_ts.to(DEVICE, non_blocking=True)

            optimizer.zero_grad(set_to_none=True)

            with torch.cuda.amp.autocast(enabled=AMP_ENABLED):
                detect_logits, pipe_logits_all, size_logits_all, pos_pred_all = model(x)

                Ld = loss_detect(detect_logits, detect)

                Lp = Ls = Lr = torch.tensor(0.0, device=DEVICE)
                for k in range(MAX_LEAKS):
                    # Slot mask: 1 where slot k contains a real leak (pipe != NONE)
                    slot_mask  = (pipe_ts[:, k] != PIPE_NONE_IDX).float()
                    slot_denom = slot_mask.sum().clamp(min=1.0)

                    Lp = Lp + (loss_pipe(pipe_logits_all[k], pipe_ts[:, k]) * slot_mask).sum() / slot_denom
                    Ls = Ls + (loss_size(size_logits_all[k], size_ts[:, k]) * slot_mask).sum() / slot_denom
                    Lr = Lr + (loss_pos(pos_pred_all[k],    pos_ts[:, k])  * slot_mask).sum() / slot_denom
    ...
    # Save bundle
    bundle = {
        "model_type": "stgcn_multi_leak_v8",
    ...
    # Window-level pipe / position evaluation across all leak slots
    model.eval()
    pipe_true_all = []
    pipe_pred_all = []
    pos_true_all  = []
    ...
    with torch.no_grad():
        for x, detect, pipe_ts, pos_ts, size_ts in test_loader:
            x = x.to(DEVICE)
            _, pipe_logits_all, _, pos_pred_all_b = model(x)

            for k in range(MAX_LEAKS):
                # Only evaluate on slots that actually have a leak
                slot_mask = (pipe_ts[:, k] != PIPE_NONE_IDX)
                if slot_mask.any():
                    pipe_true_all.append(pipe_ts[:, k][slot_mask].numpy())
                    pipe_pred_all.append(pipe_logits_all[k].argmax(dim=1)[slot_mask].cpu().numpy())
                    pos_true_all.append(pos_ts[:, k][slot_mask].numpy())
                    pos_pred_all.append(pos_pred_all_b[k][slot_mask].cpu().numpy())

    if pipe_true_all:
        pipe_true_all = np.concatenate(pipe_true_all)
        pipe_pred_all = np.concatenate(pipe_pred_all)
        f1_pipe = f1_score(pipe_true_all, pipe_pred_all, average="macro", labels=list(range(NUM_PIPES)))
        print(f"\n=== PIPE F1 MACRO (leak slots only) ===")
        print(f"F1 Macro: {f1_pipe:.4f}")

    if pos_true_all:
        pos_true_all = np.concatenate(pos_true_all)
        pos_pred_all = np.concatenate(pos_pred_all)
        mae  = float(np.mean(np.abs(pos_true_all - pos_pred_all)))
        rmse = float(np.sqrt(np.mean((pos_true_all - pos_pred_all) ** 2)))
        print(f"\n=== POSITION (leak slots only) ===")
        print(f"MAE:  {mae:.4f}")
