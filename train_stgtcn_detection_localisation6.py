...
SAVE_PATH      = "stgcn_bundle_v6_shortpath_graph.pt"
...
def build_sensor_adjacency():
    """
    Sensor-to-sensor graph with edge weights based on shortest-path hop count
    through the FULL network topology (including non-sensor intermediate nodes).

    Every sensor pair receives an edge so that sensors physically related via
    intermediate nodes are not treated as disconnected.  Edge weight is:

        w(i, j) = exp(-d(i, j) / sigma)

    where d(i, j) is the BFS hop count in the full network and sigma is the
    mean pairwise distance among all sensor pairs.
    ...
    Example distances for sensors ["P4", "Q1a", "Q3a"]:
        d(Q3a, P4)  = 1  hop
        d(Q1a, Q3a) = 4  hops  (via P2, Q2a, P3)
        d(Q1a, P4)  = 5  hops  (via P2, Q2a, P3, Q3a)
    """
    # Full physical topology — includes all nodes, not just sensors
    full_edges = [
    ...
    for i, si in enumerate(SENSOR_NAMES):
        for j, sj in enumerate(SENSOR_NAMES):
            dist_matrix[i, j] = bfs_distance(si, sj)

    # Sigma = mean of off-diagonal distances
    off_diag = dist_matrix[np.eye(N, dtype=bool) == False]
    sigma = float(np.mean(off_diag[np.isfinite(off_diag)]))

    # Build weighted adjacency: self-loops = 1.0, off-diagonal = exp(-d/sigma)
    A = np.where(dist_matrix == 0,
                 1.0,
                 np.exp(-dist_matrix / sigma)).astype(np.float32)
    A[~np.isfinite(dist_matrix)] = 0.0   # guard against disconnected nodes

    # Print adjacency for verification
    print("  Sensor-to-sensor distances:")
    for i, si in enumerate(SENSOR_NAMES):
        for j, sj in enumerate(SENSOR_NAMES):
            if i < j:
                print(f"    d({si}, {sj}) = {int(dist_matrix[i,j])} hops  "
                      f"-> w = {A[i,j]:.4f}")
...
def main():
    ...
    # Save bundle
    bundle = {
        "model_type": "stgcn_single_leak_v4",   # same architecture; graph changed
