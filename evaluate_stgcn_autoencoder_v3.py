...
Standalone evaluation of the ST-GCN autoencoder V3 (v1 architecture, stgcn_dataset_v2)
on its held-out test set.

Outputs separate files for direct comparison with v1:
  autoencoder_v3_eval_results.csv
  autoencoder_v3_reconstruction_plots/
"""
...
EVAL_CSV_PATH = "autoencoder_v3_eval_results.csv"
PLOT_DIR      = "autoencoder_v3_reconstruction_plots"
...
# ============================================================
# MODEL  (identical to v1 / v3 training script)
# ============================================================
...
def reconstruct_scenario(model, raw_signals, mu, sigma,
    ...
    Returns recon_raw : (T, N) float32 — denormalised reconstruction
    """
    ...
    # Denormalise  (mu/sigma shape: (N, 1))
    recon_raw = recon_norm_full * sigma[:, 0] + mu[:, 0]
    return recon_raw
...
def plot_scenario(scenario_id, actual_raw, recon_raw,
    ...
    fig.suptitle(f"V3 | Scenario {scenario_id}  [{label_str}]", fontsize=11, y=1.01)
    fig.tight_layout()
...
def main():
    ...
    # Summary metrics
    # ----------------------------------------------------------
    print("\n=== Per-Node Test Metrics V3 (denormalised, unmonitored nodes only) ===")
    ...
    for scn_id, raw, recon_raw, label_str in no_leak_samples + leak_samples:
        out = plot_scenario(scn_id, raw, recon_raw,
                            unmonitored_idx, sensor_names, label_str, PLOT_DIR)
