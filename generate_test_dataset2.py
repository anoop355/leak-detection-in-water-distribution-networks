...
- Leak repetitions doubled from 7 to 14 — the original 7 reps felt too thin
  for a proper generalisation test, especially for the no-leak class
- Added NOLEAK_REPETITIONS = 50 so no-leak scenarios can be generated
  independently of the leak rep count (v1 used TEST_REPETITIONS for both,
  which meant no-leak was always underrepresented)
- No-leak loop now uses NOLEAK_REPETITIONS instead of TEST_REPETITIONS
- Everything else (positions, sizes, timing, output structure) unchanged
"""
...
TEST_REPETITIONS   = 14   # doubled from v1
NOLEAK_REPETITIONS = 50   # separate count so no-leak isn't tied to leak reps
START_SPACING      = 6 * 3600
...
def main():
    ...
    n_noleak_scns = NOLEAK_REPETITIONS
    ...
    print(f"  Leak reps   : {TEST_REPETITIONS} × {START_SPACING//3600}h spacing")
    print(f"  No-leak reps: {NOLEAK_REPETITIONS}")
    ...
    # no-leak scenarios first, using NOLEAK_REPETITIONS
    print("[1/2] Generating no-leak scenarios...")
    for rep in range(NOLEAK_REPETITIONS):
