"""
generate_stgcn_dataset.py
--------------------------
Generates 2100 hydraulic scenarios (1050 leak + 1050 no-leak) using EPANET/WNTR.
14 repetitions at 12-hour spacing covers a full week.
Each scenario is 3 hours long (12 timesteps at 15-min resolution).
Leak onset at step 4 (t = 60 min): 60 min pre-leak, 120 min active.
"""
...
REPETITIONS   = 14           # 14 reps × 12h spacing = 168h (full week)
START_SPACING = 12 * 3600    # 12-hour spacing
...
SIM_A_DURATION  = 3 * 3600   # 3-hour simulation
SIM_B_DURATION  = 2 * 3600   # kept for reference only, not used
HYD_TIMESTEP    = 15 * 60    # 15-min hydraulic/report timestep
PAT_TIMESTEP    = 60         # 1-min pattern timestep
LEAK_ONSET_STEP = 4          # timestep index when leak activates

N_TIMESTEPS  = 12
T_AXIS_MIN   = [i * 15 for i in range(N_TIMESTEPS)]  # 0, 15, ..., 165

DAY_NAMES = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]

OUT_ROOT      = Path("stgcn_dataset")
...
def build_df(p: np.ndarray, q: np.ndarray) -> pd.DataFrame:
    """Build the 12-row signals DataFrame from pressure and flow arrays."""
...
def run_leak_scenario(base_path: Path, rep: int, pipe_id: int,
    ...
    """
    Run a single 3-hour simulation with the leak activating at t = 60 min.
    Uses a step demand pattern rather than two separate simulations, which
    avoids the hydraulic discontinuity from concatenating different hydraulic states.
    ...
    wn = load_and_configure(base_path, SIM_A_DURATION, rep, extra_offset_sec=0)
    ...
    # step demand: 60 zeros (pre-leak) then 120 ones (active leak)
...
def run_noleak_sim(base_path: Path, rep: int):
    """3-hour no-leak simulation. Returns (df, wn) with all 12 timesteps."""
    wn = load_and_configure(base_path, SIM_A_DURATION, rep, extra_offset_sec=0)
    clear_emitters(wn, LEAK_NODES)
    p, q = extract_signals(wntr.sim.EpanetSimulator(wn).run_sim(), N_TIMESTEPS)
...
def main():
    ...
    print("=== ST-GCN Dataset Generator ===", flush=True)
    print(
        f"Option {'A' if REPETITIONS == 28 else 'B'}: "
        f"{REPETITIONS} repetitions, {START_SPACING // 3600}h spacing",
        flush=True,
    )

    TOTAL_SCENARIOS = len(PIPES) * len(POSITIONS) * len(SIZES) * REPETITIONS * 2
    ...
    # Part 1: leak scenarios (IDs 1 - 1050)
    ...
    # Part 2: no-leak scenarios (IDs 1051 - 2100), written from cache
