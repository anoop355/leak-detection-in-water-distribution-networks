...
- Architecture updated to match train_stgtcn_v2.py: 3 ST blocks with dilated
  convolutions (dilation 1, 2, 4) instead of 2 blocks with no dilation.
  MultiTaskSTGCN now also takes num_nodes as an argument since head_in depends on it.
- Pooling changed: v1 used mean over both time and nodes; v2 pools over time only
  then flattens across nodes, giving the head a richer (N * HIDDEN_2) representation.
- Switched from argparse to hardcoded Colab paths (RESULTS_DIR, BUNDLE_PATH,
  OUTPUT_DIR, POS_TOL, DEVICE) — easier to run iteratively in a notebook.
- Added EVAL_BATCH for batched GPU inference — v1 stacked all windows into one
  tensor which caused OOM on longer scenarios.
- predict_from_signals_df now pre-allocates the window array rather than
  building a list and calling np.stack at the end.
- Removed the separate make_node_features_for_eval helper — feature building
  is now inlined in predict_from_signals_df.
- Added a progress print every 50 scenarios in main().
- model_type is now read from the bundle and added to overall_summary.json.
"""
...
SIZE_CLASSES  = 4               # exists in bundle but not used for output

# Colab paths — edit before running
RESULTS_DIR = "/content/test_data_results2"
BUNDLE_PATH = "/content/drive/MyDrive/colab_upload/multileak_stgcn_bundle_v2.pt"
OUTPUT_DIR  = "/content/drive/MyDrive/colab_upload/stgcn_v2_evaluation"
POS_TOL     = 0.25
DEVICE      = "cuda" if torch.cuda.is_available() else "cpu"

# max windows per GPU call to keep memory usage stable
EVAL_BATCH = 256
...
class MultiTaskSTGCN(nn.Module):
    def __init__(self, adj_matrix, num_nodes, hidden_1, hidden_2,
                 kernel_size=5, dropout=0.25):
        super().__init__()
        # three blocks with increasing dilation
        ...
        head_in     = num_nodes * hidden_2   # node dim preserved before flattening
        head_hidden = 64
        ...
        self.pos_head = nn.Sequential(
            nn.Linear(head_in, head_hidden), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(head_hidden, MAX_LEAKS), nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = self.block2(x)
        x = self.block3(x)
        z = x.mean(dim=1)              # pool over time: (B, N, HIDDEN_2)
        z = z.reshape(z.size(0), -1)   # flatten nodes: (B, N * HIDDEN_2)
...
def predict_from_signals_df(
    ...
    raw  = signals[feature_cols].to_numpy(dtype=np.float32)
    T, N = raw.shape
    ...
    if Tb < T:
        raise ValueError(f"Baseline template shorter than scenario: baseline={Tb}, scenario={T}")

    base  = baseline_template[:T]
    dev   = raw - base
    feats = np.stack([raw, dev], axis=-1).astype(np.float32)
    feats = (feats - mu[None, :, :]) / (sigma[None, :, :] + 1e-8)

    # pre-allocate to avoid repeated list appends
    n_windows = (T - window) // stride + 1
    X_np = np.empty((n_windows, window, N, 2), dtype=np.float32)
    for i in range(n_windows):
        start = i * stride
        X_np[i] = feats[start:start + window]

    if n_windows == 0:
        return {"predicted_leak_count": 0, "predicted_leaks": [], "num_windows": 0}

    # run in batches to keep GPU memory stable
    all_count, all_pipe, all_pos = [], [], []
    for start in range(0, n_windows, EVAL_BATCH):
        xb = torch.from_numpy(X_np[start:start + EVAL_BATCH]).to(device)
        with torch.no_grad():
            cl, pl, _sl, pp = model(xb)
        all_count.append(cl.argmax(dim=1).cpu().numpy().astype(int))
        all_pipe.append(pl.argmax(dim=2).cpu().numpy().astype(int))
        all_pos.append(pp.cpu().numpy().astype(np.float32))

    count_pred = np.concatenate(all_count, axis=0)
    pipe_pred  = np.concatenate(all_pipe,  axis=0)
    pos_pred   = np.concatenate(all_pos,   axis=0)
    ...
    return {"predicted_leak_count": int(final_count),
            "predicted_leaks": final_leaks, "num_windows": int(n_windows)}
...
def match_leaks_pipe_and_pos_no_tol(true_leaks: List[Dict], pred_leaks: List[Dict]):
    """Tolerance-free matching. Returns matched pairs for RMSE/R2 computation."""
    used_pred  = set()
    pos_errors = []
    pos_pairs  = []   # (true_pos, pred_pos, pipe_id)
    TP = 0
    true_sorted = sorted(true_leaks, key=lambda d: (int(d["pipe_id"]), float(d["position"])))
    for t in true_sorted:
        t_pipe, t_pos = int(t["pipe_id"]), float(t["position"])
...
def compute_count_classification_metrics(true_counts: List[int], pred_counts: List[int]):
    ...
    # macro F1: unweighted mean; micro F1: from aggregated TP/FP/FN
    ...
    summary = {
        ...
        "micro_f1": round(micro_f1, 4), "micro_precision": round(p_micro, 4),
        "micro_recall": round(r_micro, 4), "num_scenarios": int(len(true_arr))
    }
...
def compute_pipe_exact_match(per_rows: List[Dict]) -> Dict:
    """A scenario counts as exact match if tp_pipe == true_count and fp_pipe == fn_pipe == 0."""
    ...
    return {"pipe_exact_match_accuracy": round(exact / len(leak_rows), 4),
            "pipe_exact_match_count": exact, "n_leak_scenarios": len(leak_rows)}


def compute_pipe_confusion_normalised(pipe_stats: Dict) -> np.ndarray:
    """5x5 row-normalised confusion matrix for pipe identification."""
...
def main():
    ...
    model = MultiTaskSTGCN(
        adj_matrix=adj, num_nodes=num_nodes, hidden_1=hidden_1,
        hidden_2=hidden_2, kernel_size=kernel_size, dropout=dropout
    ).to(device)
    model.load_state_dict(bundle["model_state_dict"])
    model.eval()
    print(f"[OK] Loaded STGCN v2 from {bundle_path.name}")
    ...
    print(f"[OK] Saving results to {out_dir}")
    ...
    n_total = len(scn_dirs)
    for scn_idx, scn_dir in enumerate(scn_dirs):
        if (scn_idx + 1) % 50 == 0 or scn_idx == 0:
            print(f"  Evaluating scenario {scn_idx + 1}/{n_total} ...")
        ...
        pred = predict_from_signals_df(
            signals=signals, model=model, feature_cols=feature_cols,
            mu=mu, sigma=sigma, baseline_template=baseline_template,
        ...
        tp_pos, fp_pos, fn_pos, _pos_errs_tol = match_leaks_pipe_and_pos(
            true_leaks, pred_leaks, pos_tol=POS_TOL
        )
        ...
        update_pipe_pos(pipe_pos_stats, true_leaks, pred_leaks, pos_tol=POS_TOL)
    ...
    # pipe misclassification log — one row per imperfect scenario
    import ast
    ...
    overall["position_tolerance"] = float(POS_TOL)
    overall["window"]      = int(window)
    overall["stride"]      = int(stride)
    overall["bundle"]      = str(bundle_path.name)
    overall["model_type"]  = str(bundle.get("model_type", "stgcn_v2"))
    ...
    print(f"[INFO] position tolerance: {POS_TOL}")
