...
SAVE_PATH      = "stgcn_bundle_v7_first_diff.pt"

SENSOR_NAMES = ["P4", "Q1a", "Q3a"]
NUM_NODES = len(SENSOR_NAMES)
NODE_FEATS = 3   # [raw, deviation_from_baseline, first_difference]
...
def make_node_features(raw_signals: np.ndarray, baseline_template: np.ndarray):
    ...
    Returns:
      feats: (T, N, 3)
        ...
        feat 2 = first difference (x[t] - x[t-1]); zero-padded at t=0
    """
    ...
    # First difference: x[t] - x[t-1]; pad first timestep with zeros
    diff = np.zeros_like(raw_signals)
    diff[1:] = raw_signals[1:] - raw_signals[:-1]

    feats = np.stack([raw_signals, dev, diff], axis=-1).astype(np.float32)  # (T, N, 3)
    return feats
...
def main():
    ...
    # Save bundle
    bundle = {
        "model_type": "stgcn_single_leak_v4",   # same architecture; 3-channel input
